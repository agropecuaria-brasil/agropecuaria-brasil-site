...
// Corrige o problema de acessar links diretos (ex: /#ofertas) com Lazy Loading
const ScrollToTop = () => {
  const { pathname, hash, key } = useLocation();

  useEffect(() => {
    // 1. Se não tiver hash (âncora), rola para o topo padrão
    if (!hash) {
      window.scrollTo(0, 0);
      return;
    }

    // 2. Se tiver hash, precisamos esperar o elemento carregar (Lazy Load)
    ...
    const attemptScroll = (attempts: number) => {
      ...
      if (element) {
        // Elemento encontrado! Rola suavemente até ele com um pequeno delay visual
        setTimeout(() => {
          element.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 100);
      } else {
        // Elemento não encontrado (ainda carregando via Suspense?)
        // Tenta novamente em 100ms, até acabar as tentativas (20x = 2 segundos)
        if (attempts > 0) {
    ...
    // Inicia a busca pelo elemento (20 tentativas)
    attemptScroll(20);
