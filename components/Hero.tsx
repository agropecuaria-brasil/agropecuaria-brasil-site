...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="relative h-[100svh] md:h-[720px] flex items-center overflow-hidden bg-[#264788] pt-[136px]">
      
      {/* Background Slider */}
      <div className="absolute inset-0 z-0 bg-[#264788]">
        {heroSlides.map((slide, index) => {
          const slideStyles = getThemeStyles(slide.theme);
          const isFirstSlide = index === 0;
          const isActive = index === currentIndex;

          // CRITICAL FIX: Remove 'opacity-0' from the first slide initially to improve LCP.
          // Only apply transition logic if it's NOT the active slide.
            ...
            >
              <picture>
                  ...
                  <img 
                    ...
                    // PERFORMANCE LCP: 
                    // 1. fetchPriority="high" para a primeira imagem.
                    // 2. loading="eager" para a primeira.
                    // 3. decoding="sync" para pintar imediatamente.
                    fetchPriority={isFirstSlide ? "high" : "auto"}
                    loading={isFirstSlide ? "eager" : "lazy"}
                    decoding={isFirstSlide ? "sync" : "async"}
                    width="1440" 
                    height="720"
                    style={{ aspectRatio: '16/9' }} // Hint ao navegador
                  />
              </picture>
               
               <div className={`absolute inset-0 bg-gradient-to-r ${slideStyles.gradient}`}></div>
            </div>
          );
        })}
      </div>
